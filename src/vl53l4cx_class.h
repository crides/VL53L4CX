...
class VL53L4CX {
  public:
    ...
    {
        Serial.println("init sensor " + String(address));
      VL53L4CX_Error status = VL53L4CX_ERROR_NONE;
      VL53L4CX_Off();
      VL53L4CX_On();
      status = VL53L4CX_SetDeviceAddress(address);
        Serial.println("set addr " + String(status));

      if (status == VL53L4CX_ERROR_NONE) {
        status = VL53L4CX_WaitDeviceBooted();
        Serial.println("wait boot " + String(status));
      }

      if (status == VL53L4CX_ERROR_NONE) {
        status = VL53L4CX_DataInit();
        Serial.println("data init " + String(status));
      }
